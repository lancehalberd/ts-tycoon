...
import { actorHelpText, addActions, coreStatBonusSource, recomputeActorTags } from 'app/character';
...
import {
    Ability, ActorSource, ActorStats, AffixData,
    Area, AreaEntity, Bonuses, BonusSource,
...
function getMonsterBonuses(monster: Monster): Bonuses {
    ...
    return {
        ...
        '+range': 1,
        '+minWeaponPhysicalDamage': Math.round(.9 * (6 + 5 * growth)) * levelCoefficient,
        '+maxWeaponPhysicalDamage': Math.round(1.1 * (6 + 5 * growth)) * levelCoefficient,
...
export function initializeMonsters() {
    ...
    const fairySource = createFairySource(
        requireImage('gfx2/npcs/fairysheet3.png'),
